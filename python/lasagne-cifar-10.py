...
def load_data():
    ...
    X_test, X_val = X_test1[:5000,:,:,:],X_test1[5000:,:,:,:]
    Y_test, Y_val = Y_test1[:5000], Y_test1[5000:]

    if np.array_equal(X_val,X_test) or np.array_equal(Y_val,Y_test):
        print ("Validation and Test are exactly the same")
        exit(1)

    return dict(
        X_train=lasagne.utils.floatX(X_train),
        Y_train=Y_train.astype('int32'),
        X_val = lasagne.utils.floatX(X_val),
...
def main(n=3, num_epochs=100, model=None):
    # save_dir = 'saved_models/las-cifar10-' + str(num_epochs)
    save_dir = './saved_models/las-cifar10-100-0.5-0.0005'
    training_pahse = False
    if training_pahse:
        print("Training phase is about to be initiated")
    else:
        print("Inference phase is about to be initiated")
    ...
    if training_pahse:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in iterate_minibatches(X_train, Y_train, 100, shuffle=True, augment=False):
            ...
            np.savez(save_dir + '/model-%08d' % (epoch+1)   +'.npz', *lasagne.layers.get_all_param_values(network))
        ...
        print ("Inference for all saved models with npz format is about to be started...")
        files = glob.glob(save_dir + '/model*.npz')
        files = sorted(files)
        for f_file in files:
            print ("* processing file " + f_file)
