...
bool DNNTrainer::loadNetworkConfigBlocked() {
  ...
  net_blcoked_ = load_network_blocked((char *)configIO_->getNetConfig().c_str(), nullptr, 1);
...
void DNNTrainer::loadTrainDataBlocked(
 ...
 bool DNNTrainer::prepareBatchTrainBlocked(int start) {
   if (start + net_blcoked_->batch <= trainSize_) {
    LOG_DEBUG("Enetered to get a new batch with satrt %d and batch size %d",start,net_blcoked_->batch)
    BLOCK_ENGINE_INIT_FOR_LOOP(trainXBlocked_, x_valid_range, x_block_val_ptr, float);
    BLOCK_ENGINE_INIT_FOR_LOOP(trainYBlocked_, y_valid_range, y_block_val_ptr, float);
    BLOCK_ENGINE_INIT_FOR_LOOP(net_blcoked_->input, in_valid_range, in_block_val_ptr, float);
    BLOCK_ENGINE_INIT_FOR_LOOP(net_blcoked_->truth, out_valid_range, out_block_val_ptr, float);
    for (int i=0;i<net_blcoked_->batch;++i) {
      for (int j=0;j<WIDTH_X_HEIGHT_X_CHAN;++j) {
        BLOCK_ENGINE_COND_CHECK_FOR_LOOP_2D(trainXBlocked_, x_valid_range, x_block_val_ptr, false, x_index_var, i, j);
        BLOCK_ENGINE_COND_CHECK_FOR_LOOP_1D(net_blcoked_->input, in_valid_range, in_block_val_ptr, true, in_index_var, i*WIDTH_X_HEIGHT_X_CHAN + j);
        *(in_block_val_ptr + in_index_var - in_valid_range.block_requested_ind) = *(x_block_val_ptr+x_index_var-x_valid_range.block_requested_ind);
      }
      for (int j=0;j<NUM_CLASSES;++j) {
        BLOCK_ENGINE_COND_CHECK_FOR_LOOP_2D(trainYBlocked_, y_valid_range, y_block_val_ptr, false, y_index_var, i, j);
        BLOCK_ENGINE_COND_CHECK_FOR_LOOP_1D(net_blcoked_->truth, out_valid_range, out_block_val_ptr, true, out_index_var, i*NUM_CLASSES+j);
        *(out_block_val_ptr+out_index_var-out_valid_range.block_requested_ind) = *(y_block_val_ptr+y_index_var-y_valid_range.block_requested_ind);
      }
    }
    ...
    LOG_DEBUG("Finished to get a new batch with satrt %d and batch size %d",start,net_blcoked_->batch)
    return true;
  }
  return false;
 }
...
void DNNTrainer::trainBlocked() {
  ...
  while (get_current_batch_blocked(net_blcoked_) < net_blcoked_->max_batches) {
    ...
    LOG_INFO(
        ...
        get_current_batch_blocked(net_blcoked_), loss, avg_loss, (double)get_current_rate_blocked(net_blcoked_),
        *net_blcoked_->seen);
  }
}
...
void DNNTrainer::train(bool is_plain) {
  ...
  while (get_current_batch(net_) < net_->max_batches) {
    ...
    auto prepared = is_plain ? prepareBatchTrainPlain(start):prepareBatchTrainEncrypted(start);
    if (!prepared) {
      //intitialSort();
      start = 0;
      prepared = is_plain ? prepareBatchTrainPlain(start):prepareBatchTrainEncrypted(start);
    }
    // printf("starting iteration for batch number %d\n",
    // get_current_batch(net_));
    ...
    if (get_current_batch(net_) > 500 && get_current_batch(net_) % 500 /*epochs*/  == 1) {
      // epochs++;
      int start_test = 0;
      auto prepared_test = is_plain ? prepareBatchTestPlain(start_test):prepareBatchTestEncrypted(start_test);
      ...
      while(prepared_test) {
        ...
        free_data(testData_);
        prepared_test = is_plain ? prepareBatchTestPlain(start_test):prepareBatchTestEncrypted(start_test);
      }
      test_accuracy /= start_test;
      LOG_OUT("iteration %ld: test set accuracy %f\n",get_current_batch(net_),test_accuracy);
    }
  }
}
} // namespace darknet
