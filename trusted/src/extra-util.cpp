...
void
OCALL_LOAD_LAYER_REPRT_FRBV(int      iteration,
                            int      layer_index,
                            size_t   buff_ind,
                            uint8_t* buff,
                            size_t   size_bytes,
  ...
  if (layer_sha != nullptr) {
    ret = ocall_load_layer_report_frbv(
        iteration, layer_index, 0, nullptr, 0, layer_sha, layer_sha_len);
    CHECK_SGX_SUCCESS(ret, "ocall_load_layer_report_frbv caused problem!\n")
  }
}

void
OCALL_LOAD_LAYER_REPRT_FRBMMV(int      iteration,
                              int      layer_index,
                              size_t   buff_updates_ind,
                              uint8_t* buff_updates,
                              size_t   size_bytes_updates,
                              uint8_t* buff_updates_sha,
                              size_t   buff_updates_sha_len,
                              size_t   buff_mm_ind,
                              uint8_t* buff_mm,
                              size_t   size_bytes_mm,
                              uint8_t* buff_mm_sha,
                              size_t   buff_mm_sha_len,
                              size_t   buff_prevdelta_ind,
                              uint8_t* buff_prevdelta,
                              size_t   size_bytes_prevdelta,
                              uint8_t* buff_prevdelta_sha,
                              size_t   buff_prevdelta_sha_len) {
  sgx_status_t ret = SGX_ERROR_UNEXPECTED;
  if (buff_updates != nullptr) {
    ...
    for (int ii = 0; ii < q; ++ii) {
      ret = ocall_load_layer_report_frbmmv(
          iteration,
          layer_index,
  ...
  if (buff_mm != nullptr) {
    ...
    for (int ii = 0; ii < q; ++ii) {
      ret = ocall_load_layer_report_frbmmv(iteration,
                                           ...
                                           nullptr,
                                           0,
                                           buff_mm_ind + ii * interim_buff_len,
                                           buff_mm + ii * interim_buff_len,
                                           interim_buff_len,
                                           nullptr,
                                           0,
                                           0,
    ...
    if (r != 0) {
      ret = ocall_load_layer_report_frbmmv(iteration,
                                           ...
                                           nullptr,
                                           0,
                                           buff_mm_ind + q * interim_buff_len,
                                           buff_mm + q * interim_buff_len,
                                           r,
                                           nullptr,
                                           0,
                                           0,
  ...
  if (buff_prevdelta != nullptr) {
    ...
    for (int ii = 0; ii < q; ++ii) {
      ret = ocall_load_layer_report_frbmmv(
          ...
          nullptr,
          0,
          buff_prevdelta_ind + ii * interim_buff_len,
          buff_prevdelta + ii * interim_buff_len,
          interim_buff_len,
          nullptr,
          0);
      CHECK_SGX_SUCCESS(ret, "ocall_load_layer_report_frbmmv caused problem!\n")
    ...
    if (r != 0) {
      ret = ocall_load_layer_report_frbmmv(
          ...
          nullptr,
          0,
          buff_prevdelta_ind + q * interim_buff_len,
          buff_prevdelta + q * interim_buff_len,
          r,
          nullptr,
          0);
      CHECK_SGX_SUCCESS(ret, "ocall_load_layer_report_frbmmv caused problem!\n")
...
OCALL_SAVE_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(int      iteration,
  ...
  if (buff != nullptr) {
    ...
    for (int ii = 0; ii < q; ++ii) {
      ret = ocall_save_enclaves_layer_params_updates_frbv(
          iteration,
          layer_index,
      ...
      CHECK_SGX_SUCCESS(
          ret,
          "ocall_save_enclaves_layer_params_updates_frbv caused problem!\n")
    }
    if (r != 0) {
      ret = ocall_save_enclaves_layer_params_updates_frbv(
          iteration,
  ...
  if (aad != nullptr) {
    ret = ocall_save_enclaves_layer_params_updates_frbv(
        iteration, layer_index, 0, nullptr, 0, aad, aad_len, nullptr, 0);
    CHECK_SGX_SUCCESS(
        ret, "ocall_save_enclaves_layer_params_updates_frbv caused problem!\n")
  }
...
OCALL_LOAD_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(int      iteration,
  ...
  if (buff != nullptr) {
    ...
    int          r                = buff_len % (interim_buff_len);
    for (int ii = 0; ii < q; ++ii) {
      ret = ocall_load_enclaves_layer_params_updates_frbv(
          iteration,
          layer_index,
          start + ii * interim_buff_len,
          buff + ii * interim_buff_len,
      ...
      CHECK_SGX_SUCCESS(
          ret,
          "ocall_load_enclaves_layer_params_updates_frbv caused problem!\n")
    }
    if (r != 0) {
      ret = ocall_load_enclaves_layer_params_updates_frbv(
          iteration,
  ...
  if (aad != nullptr) {
    ret = ocall_load_enclaves_layer_params_updates_frbv(
        iteration, layer_index, 0, nullptr, 0, aad, aad_len, nullptr, 0);
    CHECK_SGX_SUCCESS(
        ret, "ocall_load_enclaves_layer_params_updates_frbv caused problem!\n")
  }
  if (layer_cmac != nullptr) {
...
bool
verify_cmac128_single_round(const uint8_t* msg,
...
fix_task_dependent_global_vars() {
  ...
  if (task_config.objPtr->security_type()
    ...
    net_context_
        = std::unique_ptr<net_context_variations>(new net_context_variations);
    *net_context_ = net_context_variations::TRAINING_INTEGRITY_LAYERED_FIT;
    net_init_loader_ptr->net_context = net_context_.get();
...
additional_auth_data
construct_aad_frbv_report_nochange_ts(uint32_t id, uint32_t ts) {
  additional_auth_data auth = {};
  auth.session_id           = session_id;
  auth.comp_compsubcomp_w_wo_ts.comp_or_subcompcom_w_ts.comp_or_compsubcomp_id
      .only_component_id.component_id
      = id;
...
additional_auth_data
construct_aad_frbv_comp_subcomp_nots(uint32_t comp_id, uint32_t subcomp_id) {
  additional_auth_data auth = {};
  auth.session_id           = session_id;
  auth.comp_compsubcomp_w_wo_ts.comp_or_subcompcom_no_ts.comp_or_compsubcomp_id
      .subcomponent_id.component_id.component_id
      = comp_id;
  auth.comp_compsubcomp_w_wo_ts.comp_or_subcompcom_no_ts.comp_or_compsubcomp_id
...
init_net() {
  if (*net_init_loader_ptr->net_context
    ...
    args.dummy = 1.0f;
    net_init_loader_ptr->invokable_params
        .init_train_privacy_integ_layered_params
        = args;
    net_init_loader_ptr->invokable.init_train_privacy_integ_layered(
        &net_init_loader_ptr->invokable_params
             .init_train_privacy_integ_layered_params);
...
send_batch_seed_to_gpu(const int iteration) {
  ...
  auto res
      = ocall_gpu_get_iteration_seed(iteration,
...
set_network_batch_randomness(const int iteration, network& net_) {
  ...
  // uint8_t*)net_.iter_batch_rng->getState().data(),sizeof(uint64_t)*16).c_str(),bytesToHexString((const
  // uint8_t*)net_.layer_rng_deriver->getState().data(),sizeof(uint64_t)*16).c_str());
}
...
init_net_train_integ_layered(
  ...
  auto net_
      = load_network((char*)archconfigs.objPtr->mutable_contents()->Data(),
                     nullptr,
                     1,
                     *net_context_,
                     verf_variations_t::FRBV);
  ...
  // LOG_WARN("FIXME!\nnetwork structure and buffers must be managed with
  // care!\n")
}

void
init_net_train_privacy_integ_layered(
    const net_init_training_privacy_integrity_layered_args* args) {
  (void)args;
...
#if defined(USE_SGX_LAYERWISE)
void
apply_weight_updates_convolutional(int                     iteration,
                                   layer&                  l,
                                   int                     layer_index,
  ...
  // load bias updates
  {
    ...
    auto l_bias_updates = l.bias_updates->getItemsInRange(start, end);
    if (*main_verf_task_variation_ == verf_variations_t::FRBV) {
      ...
      OCALL_LOAD_LAYER_REPRT_FRBMMV(iteration,
                                    layer_index,
                                    buff_ind,
                                    (uint8_t*)l_bias_updates.get(),
                                    size_bytes,
  ...
  // load weight updates
  {
    ...
    size_bytes            = (end - start) * sizeof(float);
    auto l_weight_updates = l.weight_updates->getItemsInRange(0, end);
    if (*main_verf_task_variation_ == verf_variations_t::FRBV) {
      OCALL_LOAD_LAYER_REPRT_FRBV(iteration,
                                  layer_index,
                                  buff_ind,
  ...
  if (l.batch_normalize) {
    ...
    size_bytes           = (end - start) * sizeof(float);
    auto l_scale_updates = l.scale_updates->getItemsInRange(0, end);
    if (*main_verf_task_variation_ == verf_variations_t::FRBV) {
      OCALL_LOAD_LAYER_REPRT_FRBV(iteration,
                                  layer_index,
                                  buff_ind,
    ...
    auto l_rolling_mean = l.rolling_mean->getItemsInRange(0, end);
    ...
    auto l_rolling_variance = l.rolling_variance->getItemsInRange(0, end);
  ...
  verify_sha256_mult_rounds(
      sha256_handle, nullptr, layer_sha.data(), layer_sha.size(), nullptr);
...
void
apply_weight_updates_connected(int                     iteration,
                               layer&                  l,
                               int                     layer_index,
                               sgx_sha_state_handle_t* sha256_handle) {
  ...
  // load bias updates
  {
    ...
    size_bytes          = (end - start) * sizeof(float);
    auto l_bias_updates = l.bias_updates->getItemsInRange(0, end);
    if (*main_verf_task_variation_ == verf_variations_t::FRBV) {
      OCALL_LOAD_LAYER_REPRT_FRBV(iteration,
                                  layer_index,
                                  buff_ind,
...
void
apply_weight_updates_batchnorm(int                     iteration,
                               layer&                  l,
                               int                     layer_index,
                               sgx_sha_state_handle_t* sha256_handle) {
  uint64_t               total_bytes = count_layer_paramas_bytes(l);
...
void
apply_weight_updates(int iteration, network* net) {
  ...
  if (!verify_sha256_mult_rounds(
          &sha256_handle, auth_report.data(), nullptr, 0, nullptr)) {
...
void
save_load_params_and_update_snapshot_convolutional_frbv(bool   save,
                                                        int    iteration,
                                                        layer& l,
                                                        int    layer_index) {
  sgx_cmac_state_handle_t cmac_handle = nullptr;
  ...
  if (save) {
    gen_verify_cmac128_multiple_rounds(
        true, &cmac_handle, nullptr, 0, nullptr, nullptr, 0);
  } else {
    gen_verify_cmac128_multiple_rounds(
        false, &cmac_handle, nullptr, 0, nullptr, nullptr, 0);
  }
  // bias and updates
  {
    start      = 0;
    ...
    auto l_biases = l.biases->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_biases.get(),
                                         size_bytes,
                                         nullptr,
                                         (uint8_t*)(&aad),
                                         sizeof(aad));
      OCALL_SAVE_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(iteration,
                                                    layer_index,
                                                    buff_ind,
                                                    (uint8_t*)l_biases.get(),
                                                    size_bytes,
      ...
      OCALL_LOAD_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(iteration,
                                                    layer_index,
                                                    buff_ind,
                                                    (uint8_t*)l_biases.get(),
                                                    size_bytes,
                                                    ...
                                                    nullptr,
                                                    0);
      gen_verify_cmac128_multiple_rounds(false,
                                         &cmac_handle,
                                         (uint8_t*)l_biases.get(),
                                         size_bytes,
                                         nullptr,
    ...
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         ...
                                         nullptr,
                                         0);
      OCALL_SAVE_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(
          iteration,
          layer_index,
          buff_ind,
          (uint8_t*)l_bias_updates.get(),
      ...
      OCALL_LOAD_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(
          iteration,
          layer_index,
          buff_ind,
          (uint8_t*)l_bias_updates.get(),
  ...
  // weights and updates
  {
    ...
    size_bytes     = (end - start) * sizeof(float);
    auto l_weights = l.weights->getItemsInRange(0, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_weights.get(),
                                         size_bytes,
                                         nullptr,
                                         nullptr,
                                         0);
  ...
  if (l.batch_normalize) {
    ...
    auto l_scales = l.scales->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_scales.get(),
                                         size_bytes,
                                         nullptr,
                                         nullptr,
                                         0);
    ...
    auto l_scale_updates = l.scale_updates->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_scale_updates.get(),
    ...
    // rolling mean
    auto l_rolling_mean = l.rolling_mean->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_rolling_mean.get(),
    ...
    buff_ind += size_bytes;
    // rolling variance
    auto l_rolling_variance = l.rolling_variance->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_rolling_variance.get(),
  ...
  if (save) {
    gen_verify_cmac128_multiple_rounds(
        true, &cmac_handle, nullptr, 0, layer_cmac.data(), nullptr, 0);
    OCALL_SAVE_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(iteration,
                                                  layer_index,
                                                  0,
                                                  nullptr,
    ...
    OCALL_LOAD_ENCLAVES_LAYER_PARAMS_UPDATES_FRBV(iteration,
                                                  ...
                                                  layer_cmac.data(),
                                                  layer_cmac.size());
    if (!gen_verify_cmac128_multiple_rounds(
            false, &cmac_handle, nullptr, 0, layer_cmac.data(), nullptr, 0)) {
...
void
save_load_params_and_update_snapshot_connected_frbv(bool   save,
                                                    int    iteration,
                                                    layer& l,
                                                    int    layer_index) {
  sgx_cmac_state_handle_t cmac_handle = nullptr;
  ...
  LOG_DEBUG(
      "save_load_params_and_update_snapshot_connected_frbv save = %d, finished "
      "bias and bias_updates\n",
      save)
  // weights and updates
  for (int i = 0; i < q; ++i) {
    start      = i * enclave_update_batch * l.inputs;
    ...
    size_bytes = (end - start) * sizeof(float);
    LOG_DEBUG(
        "save_load_params_and_update_snapshot_connected_frbv save = %d, i=%d, "
        "q=%d, r=%d weights in q\n",
        save,
        i,
        q,
        r)
    auto l_weights = l.weights->getItemsInRange(start, end);
    if (save) {
      gen_verify_cmac128_multiple_rounds(true,
                                         &cmac_handle,
                                         (uint8_t*)l_weights.get(),
  ...
  if (r > 0) {
    ...
    size_bytes = (end - start) * sizeof(float);
    LOG_DEBUG(
        "save_load_params_and_update_snapshot_connected_frbv save = %d, q=%d, "
        "r=%d weights in r\n",
        save,
        q,
        r)
  ...
  if (l.batch_normalize) {
    ...
    buff_ind += size_bytes;
    LOG_DEBUG(
        "save_load_params_and_update_snapshot_connected_frbv save = %d, "
        "finished bn stuff\n",
        save)
  }
  if (buff_ind != total_bytes) {
...
void
save_load_params_and_update_snapshot_batchnorm_frbv(bool   save,
                                                    int    iteration,
                                                    layer& l,
                                                    int    layer_index) {
  sgx_cmac_state_handle_t cmac_handle = nullptr;
...
void
save_load_params_and_update_snapshot_(bool save, int iteration, network* net) {
  for (int i = 0; i < net->n; ++i) {
    auto& l = net->layers[i];
    LOG_DEBUG(
        "save_load_params_and_update_snapshot_ processing layer %d of type "
        "%s\n",
        i,
        get_layer_string(l.type))
    if (l.type == CONVOLUTIONAL) {
      save_load_params_and_update_snapshot_convolutional_frbv(
          save, iteration, l, i);
    } else if (l.type == CONNECTED) {
      save_load_params_and_update_snapshot_connected_frbv(
          save, iteration, l, i);
    } else if (l.type == BATCHNORM) {
      save_load_params_and_update_snapshot_batchnorm_frbv(
          save, iteration, l, i);
    }
  }
}
...
float
train_in_enclave(int iteration, network* main_net) {
  // do forward, backward
  *main_net->seen
      = (iteration - 1) * (main_net->batch) * (main_net->enclave_subdivisions);
  main_net->train          = 1;
  float           avg_cost = 0;
  ...
  while (true) {
    ...
    setup_iteration_inputs_training_enc_layered_fit(queued_ids,
                                                    selected_ids,
                                                    main_net,
                                                    iteration,
                                                    main_net->batch,
                                                    0,
                                                    plain_dataset_size - 1);
    *main_net->seen += main_net->batch;
    ...
    if (((*main_net->seen) / main_net->batch) % main_net->enclave_subdivisions
        == 0) {
...
float
train_verify_in_enclave_frbv(int      iteration,
                             network* main_net,
                             network* verf_net) {
  // do forward, backward
  *verf_net->seen
      = (iteration - 1) * (verf_net->batch) * (verf_net->enclave_subdivisions);
  verf_net->train          = 1;
  float           avg_cost = 0;
  std::set<int>   selected_ids;
  std::queue<int> queued_ids;
  while (true) {
    ...
    *verf_net->seen += verf_net->batch;
    ...
    SET_FINISH_TIMING(SGX_TIMING_BACKWARD);
    if (((*verf_net->seen) / verf_net->batch) % verf_net->enclave_subdivisions
        == 0) {
      break;
    }
  }
  if (0) {
    std::string indices = "verification selected indices of length "
    ...
    indices = "verification selected indices from [Queue] of length "
...
void
init_randomness_frbmmv(network* net, int iteration) {
  for (int li = 0; li < net->n; ++li) {
    ...
    if (l.type == CONNECTED) {
      ...
      for (int i = 0; i < q; ++i) {
        auto l_weights = l.weights->getItemsInRange(
            i * l.enclave_layered_batch * l.inputs,
            (i + 1) * l.enclave_layered_batch * l.inputs);
        gemm(1,
             0,
             l.inputs,
             1,
             ...
             l_weights.get(),
             l.inputs,
             l.frwd_outs_rand + (i * l.enclave_layered_batch),
             1,
             1,
             l.frwd_outs_rhs,
             1);
        if (li >= 1 && net->layers[li - 1].delta) {
          gemm(0,
               ...
               l_weights.get(),
               l.inputs,
               l.bkwrd_input_delta_rand,
               1,
               1,
               l.bkwrd_input_delta_rhs + (i * l.enclave_layered_batch),
               1);
        }
      }
      if (r > 0) {
        auto l_weights = l.weights->getItemsInRange(
            q * l.enclave_layered_batch * l.inputs,
            q * l.enclave_layered_batch * l.inputs + r * l.inputs);
        gemm(1,
             0,
             l.inputs,
             1,
             ...
             l_weights.get(),
             l.inputs,
             l.frwd_outs_rand + (q * l.enclave_layered_batch),
             1,
             1,
             l.frwd_outs_rhs,
             1);
        if (li >= 1 && net->layers[li - 1].delta) {
          gemm(0,
               ...
               1,
               1,
               l.bkwrd_input_delta_rhs + (q * l.enclave_layered_batch),
               1);
        }
      }
      ...
      auto l_weights
          = l.weights->getItemsInRange(0, l.weights->getBufferSize());
...
#endif
      std::memset(
          l.frwd_outs_rhs, 0, l.c / l.groups * l.size * l.size * sizeof(float));
      gemm(0,
           0,
           1,
...
void
preload_MM_outputs_forward(network* net, int iteration, int enclave_subdiv) {
  sgx_status_t ret = SGX_ERROR_UNEXPECTED;
  for (int i = 0; i < net->n; ++i) {
    layer& l = net->layers[i];
    if (l.type == CONNECTED) {
      for (int batch = 0; batch < l.batch; ++batch) {
        auto   l_output = l.output->getItemsInRange(batch * l.outputs,
                                                  (batch + 1) * l.outputs);
        size_t start
            = (enclave_subdiv * l.batch + batch) * l.outputs * sizeof(float);
        OCALL_LOAD_LAYER_REPRT_FRBMMV(iteration,
                                      i,
                                      0,
                                      nullptr,
                                      ...
                                      nullptr,
                                      0);
        l.output->setItemsInRange(
            batch * l.outputs, (batch + 1) * l.outputs, l_output);
      }
    } else if (l.type == CONVOLUTIONAL) {
      for (int batch = 0; batch < l.batch; ++batch) {
        auto   l_output = l.output->getItemsInRange(batch * l.outputs,
...
void
preload_MM_outputs_prev_delta_backward(network* net,
                                       int      iteration,
                                       int      enclave_subdiv) {
  sgx_status_t ret = SGX_ERROR_UNEXPECTED;
  for (int i = 1; i < net->n; ++i) {
    ...
    if (l.type == CONNECTED) {
      ...
      if (net->layers[i - 1].delta) {
        ...
        for (int batch = 0; batch < l.batch; ++batch) {
          auto net_delta = net->layers[i - 1].delta->getItemsInRange(
              batch * total_elems, (batch + 1) * total_elems);
          size_t start = (enclave_subdiv * l.batch + batch) * total_elems
                         * sizeof(float);
          OCALL_LOAD_LAYER_REPRT_FRBMMV(iteration,
                                        i,
                                        0,
                                        ...
                                        0,
                                        start,
                                        (uint8_t*)net_delta.get(),
                                        total_elems * sizeof(float),
                                        nullptr,
                                        0);
          net->layers[i - 1].delta->setItemsInRange(
              batch * total_elems, (batch + 1) * total_elems, net_delta);
        }
      }
    }
...
void
preload_MM_weight_updates_backward(network* net, int iteration) {
  sgx_status_t ret = SGX_ERROR_UNEXPECTED;
  for (int i = 0; i < net->n; ++i) {
    layer& l = net->layers[i];
    if (l.type == CONNECTED) {
      ...
      for (int j = 0; j < q; ++j) {
        auto l_weight_updates = l.weight_updates->getItemsInRange(
            j * l.enclave_layered_batch * l.inputs,
            (j + 1) * l.enclave_layered_batch * l.inputs);
        OCALL_LOAD_LAYER_REPRT_FRBMMV(
            iteration,
            i,
      ...
      // new MM weight updates
      std::memset(
          l.bkwrd_weight_delta_rhs, 0, sizeof(double) * (l.n / l.groups));
      for (int j = 0; j < (l.c / l.groups * l.size * l.size); ++j) {
        l.bkwrd_weight_delta_rand[j]
            = sgx_root_rng->getRandomFloat(std::numeric_limits<float>::min(),
                                           std::numeric_limits<float>::max());
      ...
      size_t start          = (l.nbiases * sizeof(float));
      auto   weight_updates
          = l.weight_updates->getItemsInRange(0, total_elements);
      OCALL_LOAD_LAYER_REPRT_FRBMMV(iteration,
                                    i,
                                    start,
                                    (uint8_t*)weight_updates.get(),
...
float
train_verify_in_enclave_frbmmv(int      iteration,
                               network* main_net,
                               network* verf_net) {
  *verf_net->seen
      = (iteration - 1) * (verf_net->batch) * (verf_net->enclave_subdivisions);
...
float_equal(const float a, const float b) {
  if ((std::fabs(a - b) <
       ...
       0.01f * std::fmax(std::fabs(a), std::fabs(b)))
      || (std::fabs(a - b) < 0.05f)) {
    return true;
  }
  return false;
...
void
compare_param_updates_convolutional(int                     iteration,
                                    network*                verf_net,
                                    layer&                  l,
                                    int                     layer_index,
                                    sgx_sha_state_handle_t* sha256_handle) {
  ...
  // load bias updates
  {
    ...
    for (uint32_t i = 0; i < (end - start); ++i) {
      ...
      if (!float_equal(temp_vec[i], l_bias_updates[i])) {
        // LOG_ERROR("reported updates and computed for bias do not match! Layer
        // %d of type %s, at index %u with difference %f, (%f,%f)\n",
        //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
        //           - l_bias_updates[i]),temp_vec[i],l_bias_updates[i])
        // abort();
      }
    }
    LOG_DEBUG(
        "reported updates and computed for bias do not match! Layer %d of type "
        "%s with and max diff: %f\n",
        layer_index,
        get_layer_string(l.type),
        max_diff);
    max_diff = 0.0f;
  ...
  // load weight updates
  {
    ...
    for (uint32_t i = 0; i < (end - start); ++i) {
      ...
      if (!float_equal(temp_vec[i], l_weight_updates[i])) {
        // LOG_ERROR("reported updates and computed for weights do not match!
        // Layer %d of type %s, at index %u with difference %f (%f,%f)\n",
        //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
        //           - l_weight_updates[i]),temp_vec[i],l_weight_updates[i])
        // abort();
      }
    }
    LOG_DEBUG(
        "reported updates and computed for weights do not match! Layer %d of "
        "type %s with and max diff: %f\n",
        layer_index,
        get_layer_string(l.type),
        max_diff);
    max_diff = 0.0f;
  ...
  if (l.batch_normalize) {
    ...
    for (uint32_t i = 0; i < (end - start); ++i) {
      ...
      if (!float_equal(temp_vec[i], l_scale_updates[i])) {
        // LOG_ERROR("reported updates and computed for scale updates do not
        // match! Layer %d of type %s, at index %u with difference %f
        // (%f,%f)\n",
        //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
        //           - l_scale_updates[i]),temp_vec[i],l_scale_updates[i])
        // abort();
      }
    }
    LOG_DEBUG(
        "reported updates and computed for scale updates do not match! Layer "
        "%d of type %s with and max diff: %f\n",
        layer_index,
        get_layer_string(l.type),
        max_diff);
    max_diff = 0.0f;
    ...
    for (uint32_t i = 0; i < (end - start); ++i) {
      ...
      if (!float_equal(temp_vec[i], l_rolling_mean[i])) {
        // LOG_ERROR("reported updates and computed for rolling mean do not
        // match! Layer %d of type %s, at index %u with difference %f
        // (%f,%f)\n",
        //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
        //           - l_rolling_mean[i]),temp_vec[i],l_rolling_mean[i])
        // abort();
      }
    }
    LOG_DEBUG(
        "reported updates and computed for rolling mean do not match! Layer %d "
        "of type %s with and max diff: %f\n",
        layer_index,
        get_layer_string(l.type),
        max_diff);
    max_diff = 0.0f;
    ...
    for (uint32_t i = 0; i < (end - start); ++i) {
      ...
      if (!float_equal(temp_vec[i], l_rolling_variance[i])) {
        // LOG_ERROR("reported updates and computed for rolling variance do not
        // match! Layer %d of type %s, at index %u with difference %f
        // (%f,%f)\n",
        //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
        //           - l_rolling_variance[i]),temp_vec[i],l_rolling_variance[i])
        // abort();
      }
    }
    LOG_DEBUG(
        "reported updates and computed for rolling variance do not match! "
        "Layer %d of type %s with and max diff: %f\n",
        layer_index,
        get_layer_string(l.type),
        max_diff);
    max_diff = 0.0f;
...
void
compare_param_updates_connected(int                     iteration,
                                network*                verf_net,
                                layer&                  l,
                                int                     layer_index,
                                sgx_sha_state_handle_t* sha256_handle) {
  ...
  int q                    = l.outputs / enclave_update_batch;
  int r                    = l.outputs % enclave_update_batch;
  LOG_DEBUG(COLORED_STR(BRIGHT_YELLOW,
                        "layer enclave batch for connected is %d with "
                        "halved:%d,q=%d,r=%d,batch=%d,inputs=%d,outputs=%d\n"),
            l.enclave_layered_batch,
            enclave_update_batch,
            q,
            r,
            l.batch,
            l.inputs,
    ...
    for (int i = 0; i < q; ++i) {
      ...
      for (uint32_t j = 0; j < (end - start); ++j) {
        ...
        if (!float_equal(temp_vec[j], l_weight_updates[j])) {
          // LOG_ERROR("reported updates and computed for weights do not match!
          // Layer %d of type %s, at index %u with difference %f  (%f,%f)\n",
          //         layer_index,get_layer_string(l.type),j,std::fabs(temp_vec[j]
          //         - l_weight_updates[j]),temp_vec[j],l_weight_updates[j])
          // abort();
        }
      }
    }
...
void
compare_param_updates_batchnorm(int                     iteration,
                                network*                verf_net,
                                layer&                  l,
                                int                     layer_index,
                                sgx_sha_state_handle_t* sha256_handle) {
  ...
  for (uint32_t i = 0; i < (end - start); ++i) {
    ...
    if (!float_equal(temp_vec[i], l_scale_updates[i])) {
      // LOG_ERROR("reported updates and computed for scale updates do not
      // match! Layer %d of type %s, at index %u with difference %f  (%f,%f)\n",
      //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
      //           - l_scale_updates[i]),temp_vec[i],l_scale_updates[i])
      // abort();
    }
  ...
  LOG_DEBUG(
      "reported updates and computed for scale updates do not match! Layer %d "
      "of type %s with and max diff: %f\n",
      layer_index,
      get_layer_string(l.type),
      max_diff);
  ...
  for (uint32_t i = 0; i < (end - start); ++i) {
    ...
    if (!float_equal(temp_vec[i], l_rolling_mean[i])) {
      // LOG_ERROR("reported updates and computed for rolling mean do not match!
      // Layer %d of type %s, at index %u with difference %f  (%f,%f)\n",
      //           layer_index,get_layer_string(l.type),i,std::fabs(temp_vec[i]
      //           - l_rolling_mean[i]),temp_vec[i],l_rolling_mean[i])
      // abort();
  ...
  LOG_DEBUG(
      "reported updates and computed for rolling variance do not match! Layer "
      "%d of type %s with and max diff: %f\n",
      layer_index,
      get_layer_string(l.type),
      max_diff);
...
bool
compare_param_updates_with_report_frbv(int iteration, network* verf_net) {
  ...
  for (int i = 0; i < verf_net->n; ++i) {
    ...
    if (l.type == CONVOLUTIONAL) {
      compare_param_updates_convolutional(
          iteration, verf_net, l, i, &sha256_handle);
    } else if (l.type == CONNECTED) {
      compare_param_updates_connected(
          iteration, verf_net, l, i, &sha256_handle);
    } else if (l.type == BATCHNORM) {
      compare_param_updates_batchnorm(
          iteration, verf_net, l, i, &sha256_handle);
    }
  }
  if (!verify_sha256_mult_rounds(
...
verify_task_frbv() {
  ...
  if (found_task) {
    ...
    if (iteration == 1) {
      ...
      LOG_DEBUG(
          COLORED_STR(
              RED, "loading weights and updates from previous iteration: %d\n"),
          iteration - 1)
      save_load_params_and_update_snapshot_(
          false, iteration - 1, verf_network_.get());
    }

    // do forward, backward
    SET_START_TIMING(SGX_TIMING_ONEPASS);
    auto avg_cost = train_verify_in_enclave_frbv(
        iteration, network_.get(), verf_network_.get());
    SET_FINISH_TIMING(SGX_TIMING_ONEPASS);
    LOG_DEBUG(
        COLORED_STR(BRIGHT_RED,
                    "Verification: average cost for iteration %d is : %f\n"),
        iteration,
        avg_cost)
    // compare weight updates with with reported ones

    if (!compare_param_updates_with_report_frbv(iteration,
                                                verf_network_.get())) {
    } else {
    ...
    CHECK_SGX_SUCCESS(
        ret, "ocall_delete_snapshots_after_verification caused problem!\n")
  }
}
...
verify_task_frbmmv() {
  ...
  if (found_task) {
    ...
    // LOG_DEBUG("here!re 3\n")
    SET_START_TIMING(SGX_TIMING_ONEPASS);
    auto avg_cost = train_verify_in_enclave_frbmmv(
        iteration, network_.get(), verf_network_.get());
    // LOG_DEBUG("here!re 4\n")
    SET_FINISH_TIMING(SGX_TIMING_ONEPASS);
    LOG_DEBUG(
...
setup_iteration_inputs_training(std::queue<int>& queued_ids,
  ...
  // LOG_DEBUG("preparing inputs for verification network in iteration
  // %d,low:%d,high:%d\n",iteration,low,high)
  ...
  LOG_DEBUG(
      "Started filling setup_iteration_inputs_training for iteration %d\n",
      iteration)
  while (!selected_ids.empty()) {
    // for (const auto id : selected_ids) {
    int id = selected_ids.front();
    // per input
    auto net_input = net->input->getItemsInRange(
        (ind * required_img_elems), ((ind + 1) * required_img_elems));
    auto net_truth = net->truth->getItemsInRange(
        (ind * required_lbl_elems), ((ind + 1) * required_lbl_elems));
    ...
    if (!verify_cmac128_single_round(auth_buff->content()->Data(),
                                     auth_buff->content()->size(),
                                     auth_buff->mac()->Data(),
                                     (uint8_t*)&auth,
                                     sizeof(auth))) {
    ...
    std::memcpy(
        net_input.get(), imglabel->img_content()->Data(), required_img_bytes);

    if (net->truth) {
      std::memcpy(net_truth.get(),
                  imglabel->label_content()->Data(),
                  required_lbl_byets);
    }
    // per input
    net->input->setItemsInRange((ind * required_img_elems),
                                ((ind + 1) * required_img_elems),
                                net_input);
    net->truth->setItemsInRange((ind * required_lbl_elems),
                                ((ind + 1) * required_lbl_elems),
                                net_truth);
    ++ind;
    selected_ids.pop();
  }
...
void
setup_iteration_inputs_training_enc_layered_fit(
    std::queue<int>& queued_ids,
    std::set<int>&   selected_ids_prev,
    network*         net,
    int              iteration,
  ...
  while (!selected_ids.empty()) {
    ...
    auto res = sgx_rijndael128GCM_decrypt(
        &enclave_ases_gcm_key,
...
// }
#if defined(USE_SGX_LAYERWISE)
    std::memcpy(net_input.get() + (ind * required_img_elems),
                imglabel->img_content()->Data(),
                required_img_bytes);
    if (net->truth) {
      std::memcpy(net_truth.get() + (ind * required_lbl_elems),
                  imglabel->label_content()->Data(),
                  required_lbl_byets);
    }
#elif defined(USE_SGX_PURE)
    std::memcpy(net->input + (ind * required_img_elems),
                imglabel->img_content()->Data(),
                required_img_bytes);
    if (net->truth) {
      std::memcpy(net->truth + (ind * required_lbl_elems),
                  imglabel->label_content()->Data(),
                  required_lbl_byets);
    }
#endif
...
#if defined(USE_SGX_LAYERWISE)
void
start_training_verification_frbv(int iteration) {
  ...
  auto ret = ocall_gpu_train_report_frbv(
      iteration, report.data(), SGX_SHA256_HASH_SIZE);
  ...
  ret = ocall_save_auth_report_frbv(
      iteration, auth_report.data(), auth_report.size());
  CHECK_SGX_SUCCESS(ret, "ocall_save_auth_report_frbv caused problem!\n")
  // apply the computed updates (in GPU) that will be added up to weights step
  // in enclave
  ...
  // in case you do not want GPU performing update phase, let the GPU get the
  // updates from SGX
  ret = ocall_use_sgx_new_weights_momentum_grads(iteration);
  CHECK_SGX_SUCCESS(
      ret, "ocall_use_sgx_new_weights_momentum_grads caused problem!\n")
  ...
  if (1) {
    ...
    std::memcpy(
        task.task.frvb_task.reported_hash, report.data(), SGX_SHA256_HASH_SIZE);
    if (verf_rand <= net_init_loader_ptr->invokable_params
                         .init_train_integ_layered_params.verif_prob) {
...
void
start_training_verification_frbmmv(int iteration) {
  std::vector<uint8_t> report(SGX_SHA256_HASH_SIZE, 0);
  send_batch_seed_to_gpu(iteration);
  set_network_batch_randomness(iteration, *network_);
  setup_layers_iteration_seed(*network_, iteration);
  auto ret = ocall_gpu_train_report_frbmmv(
      iteration, report.data(), SGX_SHA256_HASH_SIZE);
  ...
  CHECK_SGX_SUCCESS(ret, "sgx_cmac128_close caused problem!\n")
  // save_auth_step_report outside
  ret = ocall_save_auth_report_frbmmv(
      iteration, auth_report.data(), auth_report.size());
...
void
start_training_in_sgx(int iteration) {
#if defined(USE_SGX_LAYERWISE)
  set_network_batch_randomness(iteration, *network_);
  setup_layers_iteration_seed(*network_, iteration);
