...
void
ecall_setup_channel(uint64_t chan_id, int channel_type) {
#if 0
...
void
ecall_tearup_channel(uint64_t chan_id) {
}

void
ecall_send_to_channel(uint64_t chan_id, unsigned char *buff, size_t len) {
...
void
ecall_receive_from_channel(uint64_t chan_id, unsigned char *buff, size_t len) {
 ...
 */
void
ecall_test_long_buffer_encrypt(size_t complete_len) {
  ...
  for (size_t i = 0; i < q; ++i) {
    ...
    status = ippsAES_GCMEncrypt(
        (const uint8_t *)&nums[0], (uint8_t *)&enc_nums[0], buffersize, pAES);
  ...
  if (r > 0) {
    ...
    status = ippsAES_GCMEncrypt(
        (const uint8_t *)&nums[0], (uint8_t *)&enc_nums[0], r, pAES);
    if (!checkStatus("ippsAES_GCMEncrypt", ippStsNoErr, status)) {
      abort();
    }
    if (first) {
      // ocall with mac
      ret = ocall_test_long_buffer_encrypt_store(
          1, 0, complete_len, &enc_nums[0], r, iv, NULL);
      CHECK_SGX_SUCCESS(ret, "sending encrypted buffer caused problem")
      first = 0;
    } else {
      ret = ocall_test_long_buffer_encrypt_store(
          0, 0, complete_len, &enc_nums[0], r, NULL, NULL);
      CHECK_SGX_SUCCESS(ret, "sending encrypted buffer caused problem")
    }
  }
  ...
  ret = ocall_test_long_buffer_encrypt_store(
      0, 1, complete_len, NULL, 0, NULL, mac);
...
void
ecall_test_long_buffer_decrypt(size_t complete_len) {
  if (complete_len % sizeof(float) != 0) {
    LOG_ERROR("complete_len must be divisible by size of float\n")
    abort();
  }
  ...
  for (size_t i = 0; i < q; ++i) {
    ...
    CHECK_SGX_SUCCESS(ret, "retrieve caused problem\n")
    status = ippsAES_GCMDecrypt(
        (const uint8_t *)&enc_nums[0], (uint8_t *)&nums[0], buffersize, pAES);
  ...
  if (r > 0) {
    ret = ocall_test_long_buffer_decrypt_retrieve(
        0, q * buffersize, &enc_nums[0], r, NULL, NULL);
    CHECK_SGX_SUCCESS(ret, "retrieve caused problem\n")
    status = ippsAES_GCMDecrypt(
        (const uint8_t *)&enc_nums[0], (uint8_t *)&nums[0], r, pAES);
    if (!checkStatus("ippsAES_GCMDecrypt", ippStsNoErr, status)) {
      abort();
    }
    for (size_t j = 0; j < r / sizeof(float); ++j) {
...
void
ecall_enclave_init(unsigned char *common_run_config, size_t len) {
...
void
ecall_init_ptext_imgds_blocking2D(int single_size_x_bytes,
  ...
  LOG_DEBUG(
      "Blocked buffer for plaintext X instantiated! for %d images with "
      "%d pixels\n",
      total_items,
      num_pixels);
  ...
  LOG_DEBUG(
      "Blocked buffer for plaintext Y instantiated! for %d images with "
      "%d labels\n",
      total_items,
      num_labels);
  ...
  for (int64_t i = 0; i < total_items; ++i) {
    ...
    for (int64_t j = 0; j < num_pixels; ++j) {
      BLOCK_ENGINE_COND_CHECK_FOR_LOOP_2D(
          plain_ds_2d_x, x_valid_range, block_val_x, true, current_ind, i, j)
    ...
    for (int64_t j = 0; j < num_labels; ++j) {
      BLOCK_ENGINE_COND_CHECK_FOR_LOOP_2D(
          plain_ds_2d_y, y_valid_range, block_val_y, true, current_ind, i, j)
...
void
ecall_init_ptext_imgds_blocking1D(int single_size_x_bytes,
                                  int single_size_y_bytes,
                                  int total_items) {
  LOG_ERROR("This part needs change!\n");
  abort();
...
void
ecall_assign_random_id(unsigned char *tr_records, size_t len) {
  LOG_ERROR("This part needs change!\n");
  abort();
...
simple_dnnl_mult() {
  ...
  try {
    dnnl_sgemm(
        'N', 'N', M, N, K, 1.0, a.data(), K, b.data(), N, 1.0, c.data(), N);
  } catch (dnnl::error &e) {
...
//   int i;
//
//   for (i = starterM; i < starterM + (((M - starterM) / TILE_M) * TILE_M);
//        i += TILE_M) {
//     int j, k;
//     int i_d, k_d;
//
...
//         // L2 - 9 bits tag [14:6] - cache size 256 KB, conflict for each 32
//         KB
//         // L3 - 13 bits tag [18:6] - cache size 8 MB, conflict for each 512
//         KB
//
//         __m256 result256;
...
void
ecall_handle_gemm_cpu_first_mult(int thread_num) {
...
void
ecall_handle_gemm_all(int thread_num) {
#if defined(USE_SGX) && defined(USE_GEMM_THREADING_SGX)
  if (thread_num < 0 || thread_num >= per_thr_params.size()) {
    LOG_DEBUG("GEMM threading was called with wrong param");
    abort();
  ...
  task.second._a.store(thread_task_status_t::in_progress);
  // LOG_DEBUG("task started for thread num: %d, starterM=%d, M=%d, N=%d, K=%d,
  // ALPHA=%f, A=%p, lda=%d, B=%p, ldb=%d, C=%p, ldc=%d\n",
  //           thread_num,
  //           task.first.starterM, task.first.M, task.first.N, task.first.K,
  //           task.first.ALPHA, task.first.A, task.first.lda, task.first.B,
  //           task.first.ldb, task.first.C, task.first.ldc)
  ...
  //   &(task.first.BETA), (task.first.C+task.first.starterM),
  //   &(task.first.ldc));
...
// gemm_nn(M, N, K, ALPHA, A, lda, B, ldb, C, ldc);
...
#else
    gemm_nn_fast(task.first.starterM,
                 task.first.M,
                 task.first.N,
                 task.first.K,
                 task.first.ALPHA,
    ...
    for (i = task.first.starterM; i < task.first.M; ++i) {
      for (k = 0; k < task.first.K; ++k) {
        ...
        for (j = task.first.starterN; j < task.first.N; ++j) {
          task.first.C[i * task.first.ldc + j]
              += A_PART * task.first.B[k * task.first.ldb + j];
        }
      }
    }
  } else if (!task.first.TA && task.first.TB) {
    ...
    for (i = task.first.starterM; i < task.first.M; ++i) {
      for (j = task.first.starterN; j < task.first.N; ++j) {
        float sum = 0;
        for (k = 0; k < task.first.K; ++k) {
          sum += task.first.ALPHA * task.first.A[i * task.first.lda + k]
                 * task.first.B[j * task.first.ldb + k];
        }
    ...
    for (i = task.first.starterM; i < task.first.M; ++i) {
      for (j = task.first.starterN; j < task.first.N; ++j) {
        float sum = 0;
        for (k = 0; k < task.first.K; ++k) {
          sum += task.first.ALPHA * task.first.A[i + k * task.first.lda]
                 * task.first.B[k + j * task.first.ldb];
        }
        task.first.C[i * task.first.ldc + j] += sum;
      }
    }
  }
  // LOG_DEBUG("task finished for thread num: %d, starterM=%d, M=%d, N=%d, K=%d,
  // ALPHA=%f, A=%p, lda=%d, B=%p, ldb=%d, C=%p, ldc=%d\n",
  //           thread_num,
  //           task.first.starterM, task.first.M, task.first.N, task.first.K,
  //           task.first.ALPHA, task.first.A, task.first.lda, task.first.B,
...
void
ecall_handle_fill_cpu(int thread_num) {
  // #if defined(USE_SGX) && defined (USE_GEMM_THREADING_SGX)
  // if (thread_num < 0 || thread_num >=
  // cpu_same_src_dest_per_thr_params.size()) {
...
void
ecall_handle_scale_cpu(int thread_num) {
  // #if defined(USE_SGX) && defined (USE_GEMM_THREADING_SGX)
  // if (thread_num < 0 || thread_num >=
  // cpu_same_src_dest_per_thr_params.size()) {
  ...
  //   auto& task = cpu_same_src_dest_per_thr_params[thread_num];
  //   if (task.second._a.load() != thread_task_status_t::not_started) {
  //     LOG_DEBUG("scale cpu threading was called for the same thread
  //     again!!\n");
  //   }
  //   task.second._a.store(thread_task_status_t::in_progress);
  //   for(int i = task.first.starterN; i < task.first.N; ++i) {
